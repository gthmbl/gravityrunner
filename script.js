document.addEventListener("DOMContentLoaded", function () {
  ...
  function createObstacle() {
    ...
    obstacle.style.left = gameContainer.getBoundingClientRect().right + "px";
  ...
  function updateGame() {
    ...
    obstacles.forEach((obstacle) => {
      ...
      if (
        ...
        const intersectingUp =
          gravity < 0 &&
          ballRect.top <= obstacleRect.bottom &&
          Math.abs(ballRect.top - obstacleRect.bottom) > 5;

        const intersectingDown =
          gravity > 0 &&
          ballRect.bottom >= obstacleRect.top &&
          Math.abs(ballRect.bottom - obstacleRect.top) > 5;

        const intersectingVertically = intersectingUp || intersectingDown;

        // if the ball is bumping to the side of the obstacle
        if (ballRect.right >= obstacleRect.left && intersectingVertically) {
          stuck = true;
          ...
          //     stuck = true;
          //     ballCurrentX = obstacleRect.left - ballRect.width;
          //   } else if (gravity < 0 && ballRect.top < obstacleRect.bottom) {
          //     stuck = true;
          //     ballCurrentX = obstacleRect.left - ballRect.width;
          //   }
          //   else if (gravity > 0 && ballRect.bottom >= obstacleRect.top) {
          //     ballY = obstacleRect.top - ballRect.height;
          //     velocityY = 0;
          //     rolling = true;
          //   }
